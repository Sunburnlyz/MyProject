...
def watermark_create(image_path, watermark_path, out_path, alpha=0.05):
    ...
    watermark_resized = cv2.resize(watermark, (image.shape[1] // 2, image.shape[0] // 2))

    coeffs = pywt.dwt2(image, 'haar')
    LL, (LH, HL, HH) = coeffs
    LH_wm = LH + alpha * watermark_resized
...
def crop(image_path):
    image = cv2.imread(image_path, cv2.IMREAD_COLOR)
    cropped_image = image[100:200, 50:300]
